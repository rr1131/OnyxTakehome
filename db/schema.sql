...
-- Prices/cash are USD with six decimal places; quantities are whole contracts.
...
-- Buy-only execution. The future server order handler must authenticate the
...
RETURNS TABLE (order_id uuid, fill_id uuid, balance numeric)
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  ...
  RETURN QUERY SELECT v_order_id, v_fill_id, v_balance;
...
REVOKE ALL ON FUNCTION public.execute_paper_order(text, text, text, integer, numeric)
  FROM PUBLIC;
